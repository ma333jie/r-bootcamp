...
print(c(val, Val)) # add all numbers togehters 
```

We can work with (and store) sequences and repetitions
```{r}
mySeq <- 1:6 # assign variable 
mySeq
...
nObs <- 1000
mySample <- sample(1:1000, 100, replace = TRUE) # random number 
...
vals[3:5]
vals[c(1, 3, 6)]
vals[-c(1, 3, 6)] # except the  1,3,6 indices 
...
##".." means go up and go down. 
#######################################################################

## create a simple vector from the Gapminder dataset
gdp <- gap$gdpPercap 
# the orignial datat set will not be changed 
gdp[1:10] #get single row data 
...
vec1 != vec2
## careful: 
vec1 = vec2 #assign and copy value 
identical(vec1, vec2)
